...
def compileCode(lang,filename):  # Code is in 'current.???' according to language used.
	if lang == "CPP":
		exit_code = os.system("g++ -O2 --std=c++11 -s -static -o current.out "+filename)
		if exit_code != 0:
			return "ERROR",""
	if lang == "C":
		exit_code = os.system("gcc -s -static -O3 -o current.out "+filename+" -lm")
		if exit_code != 0:
			return "ERROR",""
	if lang =="JAVA":
...
def runCode(pid,lang,exename):       # Executable is in corresponding file as generated in compileCode
	...
	for test in testcases[pid]:
		try:
			...
			cmd = "./box.out -a3 -f -m %d -t %d -i %s -o %s %s" % (test[3]*1024,test[4]*timeFactor,infile,"temp.out",exename)
			print cmd
			...
			return "Unknown error, OJ didn't understand what you were trying to do!",score
